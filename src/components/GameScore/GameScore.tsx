    ...
    return (
        <>
            ...
            <div className="flex justify-between items-center mb-4 mt-2 relative">
                {players.map(player => (
                    <div className={"text-center w-[44%] lg:w-5/12 2xl:w-2/6 text-gray-500 dark:text-white" + (player.id === currentPlayer.id ? ' !text-blue-700 dark:!text-blue-300' : '')} key={player.id}>
                        <span title={player.name} className={"cursor-default text-xs min-[400px]:text-sm 2xl:text-base duration-200 block truncate" + (player.name === currentPlayer.name ? ' scale-[1.05]' : '')}>{player.name}</span>
